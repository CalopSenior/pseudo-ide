...
function __vp(valor, tipos, metodo, param) {
    ...
    const ok = tipos.some(tipo => {
        switch (tipo) {
            case 'numero':   return typeof valor === 'number' && !isNaN(valor);
            case 'inteiro':  return typeof valor === 'number' && Number.isInteger(valor);
            case 'texto':    return typeof valor === 'string';
            case 'booleano': return typeof valor === 'boolean';
            case 'funcao':   return typeof valor === 'function';
            case 'Lista':    return valor instanceof PseudoLista;
            case 'Conjunto': return valor instanceof PseudoConjunto;
            case 'Caracter': return valor instanceof PseudoCaracter || typeof valor === 'string';
            case 'Numero':   return valor instanceof PseudoNumero;
    ...
    if (!ok) {
        ...
        throw new Error(
            ...
            (valor !== null && valor !== undefined ? ` (${String(valor).slice(0, 30)})` : '') + '.'
        );
    }
}
...
/* ============================================================
   3. RASTREAMENTO DE LINHA DE ERRO
      O eval do usuário é envolvido em:
        (async function __A__(){ try { \n
          [LINHA 2] código do usuário...
        } catch ... })()
      Portanto: linha_usuario = linha_eval - OFFSET_HEADER
   ============================================================ */
const _EVAL_HEADER_LINES = 2; // linhas antes do código do usuário no ctx

function _extrairLinhaErro(stack) {
    if (!stack) return null;
    // Chrome/Edge/Node: "<anonymous>:LINE:COL"
    // Firefox: "Function:LINE:COL" ou "eval:LINE:COL"
    const padroes = [
        /<anonymous>:(\d+):\d+/,
        /eval(?::| code):(\d+)(?::\d+)?/,
        /Function:(\d+):\d+/,
    ...
    for (const re of padroes) {
        ...
        if (m) {
            const linhaEval = parseInt(m[1]);
            const linhaUsuario = linhaEval - _EVAL_HEADER_LINES;
            return linhaUsuario > 0 ? linhaUsuario : null;
        }
    }
    return null;
}
...
class PseudoCaracter {
    constructor(v) {
        __vp(v, ['texto', 'numero', 'booleano', 'Caracter'], 'metodos.texto', 'valor');
        this._v = String(v instanceof PseudoCaracter ? v._v : v);
    }
    maiusculo()   { return new PseudoCaracter(this._v.toUpperCase()); }
    minusculo()   { return new PseudoCaracter(this._v.toLowerCase()); }
    capitalizar() { return new PseudoCaracter(this._v.charAt(0).toUpperCase() + this._v.slice(1).toLowerCase()); }
    contem(s)     { __vp(s, ['texto','Caracter'], 'caracter.contem', 's'); return this._v.includes(String(s)); }
    tamanho()     { return this._v.length; }
    inverter()    { return new PseudoCaracter(this._v.split('').reverse().join('')); }
    aparar()      { return new PseudoCaracter(this._v.trim()); }
    mesclar(s)    { return new PseudoCaracter(this._v + String(s)); }
...
/* --- PseudoLista --- */
class PseudoLista {
    constructor(itens) { this._v = [...itens]; }
    adicionar(item) { this._v.push(item); return this; }
    remover(item)   { const i = this._v.indexOf(item); if (i > -1) this._v.splice(i, 1); return this; }
    obter(i)        {
        __vp(i, ['inteiro'], 'lista.obter', 'indice');
        if (i < 0 || i >= this._v.length)
            throw new Error(`lista.obter(): índice ${i} fora do intervalo [0, ${this._v.length - 1}].`);
        return this._v[i];
    }
    tamanho()       { return this._v.length; }
    ordenar()       { this._v.sort((a, b) => (a > b ? 1 : a < b ? -1 : 0)); return this; }
    contem(item)    { return this._v.includes(item); }
    toString()      { return '[' + this._v.map(String).join(', ') + ']'; }
    valueOf()       { return this._v; }
}
...
class PseudoMapa {
    ...
    definir(chave, val)  { this._v.set(chave, val); return this; }
    obter(chave)         { return this._v.get(chave); }
    remover(chave)       { this._v.delete(chave); return this; }
    tem(chave)           { return this._v.has(chave); }
    toString()           { return '[Mapa]'; }
}

/* --- PseudoConjunto --- */
class PseudoConjunto {
    constructor()   { this._v = new Set(); }
    adicionar(v)    { this._v.add(v);    return this; }
    remover(v)      { this._v.delete(v); return this; }
    tem(v)          { return this._v.has(v); }
    tamanho()       { return this._v.size; }
    ...
    intersecao(outro) {
        ...
        const r = new PseudoConjunto();
        this._v.forEach(v => { if (outro._v.has(v)) r._v.add(v); });
        return r;
    }
    diferenca(outro) {
...
class PseudoNumero {
    constructor(v) {
        if (v === true  || v === 'verdadeiro') { this._v = 1; return; }
        if (v === false || v === 'falso')      { this._v = 0; return; }
        if (typeof v !== 'number' && typeof v !== 'string')
            throw new Error(`metodos.numero(): não é possível converter ${typeof v} em número.`);
        const n = Number(v);
        if (isNaN(n)) throw new Error(`metodos.numero(): "${v}" não é um número válido.`);
        this._v = n;
    }

    /** Retorna 1 (positivo), -1 (negativo) ou 0 (zero). */
    sinal()     { return Math.sign(this._v); }

    /** Converte para inteiro truncando (sem arredondar). verdadeiro→1, falso→0. */
    int()       { return Math.trunc(this._v); }
    ...
    fatorar(modo = 0) {
        ...
        if (n < 2) throw new Error(`fatorar(): o valor deve ser um inteiro ≥ 2 (recebido: ${this._v}).`);
        if (n > 1e12) throw new Error('fatorar(): valor muito grande para fatoração (máx: 10¹²).');
        ...
        for (let d = 2; d * d <= rest; d++) {
            if (rest % d === 0) {
                let e = 0;
                while (rest % d === 0) { e++; rest = Math.trunc(rest / d); }
                fatores.push([d, e]);
            }
        }
        ...
        if (modo === 1) {
            const sup = ['⁰','¹','²','³','⁴','⁵','⁶','⁷','⁸','⁹'];
            return fatores.map(([b, e]) =>
                b + String(e).split('').map(d => sup[+d]).join('')
            ).join(' × ');
        }
...
function _c()  { return document.getElementById('console-saida'); }
function _sc() { const c = _c(); c.scrollTop = c.scrollHeight; }
function _e(s) {
    return String(s)
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
...
function _renderizarTabela(cabecalhos, linhas, colorFn = null) {
    ...
    let h = `<table style="border-collapse:collapse;font-family:'JetBrains Mono',monospace;` +
    ...
    cabecalhos.forEach(c => {
        h += `<th style="border-bottom:1px solid #2a2d40;padding:7px 14px;color:#7c83ff;` +
    ...
    linhas.forEach((row, ri) => {
        ...
        row.forEach((celula, ci) => {
            ...
            h += `<td style="border-bottom:1px solid #1e2135;padding:6px 14px;` +
...
const Bibliotecas = {
    ...
    mat: {
        ...
        get e()           { return Math.E; },
        get Infinito()    { return Infinity; },
        get NegInfinito() { return -Infinity; },

        abs:   x => { __vp(x, ['numero'], 'mat.abs',   'x'); return Math.abs(x); },
        arred: x => { __vp(x, ['numero'], 'mat.arred', 'x'); return Math.round(x); },
        piso:  x => { __vp(x, ['numero'], 'mat.piso',  'x'); return Math.floor(x); },
        teto:  x => { __vp(x, ['numero'], 'mat.teto',  'x'); return Math.ceil(x); },

        max: (a, b) => { __vp(a,['numero'],'mat.max','a'); __vp(b,['numero'],'mat.max','b'); return Math.max(a, b); },
        min: (a, b) => { __vp(a,['numero'],'mat.min','a'); __vp(b,['numero'],'mat.min','b'); return Math.min(a, b); },

        sen:  x => { __vp(x,['numero'],'mat.sen','x');  return Math.sin(x); },
        cos:  x => { __vp(x,['numero'],'mat.cos','x');  return Math.cos(x); },
        tan:  x => { __vp(x,['numero'],'mat.tan','x');  return Math.tan(x); },

        ln:   x => { __vp(x,['numero'],'mat.ln','x');   return Math.log(x); },
        log2: x => { __vp(x,['numero'],'mat.log2','x'); return Math.log2(x); },
        log10:x => { __vp(x,['numero'],'mat.log10','x');return Math.log10(x); },
        log:  (x, base) => {
            __vp(x,   ['numero'], 'mat.log', 'x');
            __vp(base,['numero'], 'mat.log', 'base');
            if (base <= 0 || base === 1) throw new Error('mat.log(): base deve ser > 0 e ≠ 1.');
        ...
        aleatorio: (mn, mx) => {
            ...
            if (mn > mx) throw new Error('mat.aleatorio(): min não pode ser maior que max.');
    ...
    tabular: {
        separador: () => {
            ...
            el.style.cssText = 'border-top:1px dashed #353850;margin:10px 0;opacity:.7;';
            _c().appendChild(el); _sc();
        },

        tabela: (cab, rows) => {
            const norm = v => Array.isArray(v) ? v : (v instanceof PseudoLista ? v._v : [String(v)]);
        ...
        progresso: (val, max) => {
            ...
            const tot = 30, ch = Math.round(pct * tot);
        ...
        tabelaVerdade: (expressoes, variaveis, mostrarIntermediarias = false) => {
            ...
            if (vars.length === 0) throw new Error('tabular.tabelaVerdade(): A lista de variáveis não pode estar vazia.');
            if (vars.length > 8)  throw new Error('tabular.tabelaVerdade(): Máximo de 8 variáveis permitidas (2⁸ = 256 linhas).');

            // 2. Normalizar expressões (aceita string única ou Lista)
            let exprs_input = [];
            if (expressoes instanceof PseudoLista)   exprs_input = expressoes._v.map(String);
            else if (Array.isArray(expressoes))       exprs_input = expressoes.map(String);
            else                                      exprs_input = [String(expressoes)];
            if (exprs_input.length === 0) throw new Error('tabular.tabelaVerdade(): Nenhuma expressão fornecida.');
                    ...
                    for (let i = 0; i < ex.length; i++) {
                        if (ex[i] === '(') {
                            ...
                            for (let j = i + 1; j < ex.length; j++) {
                                if (ex[j] === '(') depth++;
                                if (ex[j] === ')') depth--;
                                if (depth === 0) { subs.add(ex.substring(i, j + 1).trim()); break; }
                            }
                        }
                    }
                });
            ...
            exprs_input.forEach(ex => { if (!colunasExpr.includes(ex.trim())) colunasExpr.push(ex.trim()); });

            // 4. Compilar uma função avaliadora por coluna de expressão
            const _traduzirExpr = ex => ex
            ...
            cab.forEach(h => {
                html += `<th style="border-bottom:1px solid #2a2d40;padding:7px 14px;color:#7c83ff;` +
            ...
            linhas.forEach((linha, ri) => {
                ...
                linha.forEach((cel, ci) => {
                    ...
                    html += `<td style="border-bottom:1px solid #1e2135;padding:6px 14px;` +
    ...
    /* ================================================================
       ...
       ================================================================ */
    metodos: {
        texto: v   => new PseudoCaracter(v),
        int:  v   => { const n = parseInt(v, 10); if (isNaN(n)) throw new Error(`metodos.inteiro(): "${v}" não pode ser convertido para inteiro.`); return n; },
        re:     v   => { const n = parseFloat(v);   if (isNaN(n)) throw new Error(`metodos.real(): "${v}" não pode ser convertido para real.`);    return n; },
    ...
    calculo: {
        ...
        limite: (fn, ponto) => {
            ...
            if (!isFinite(L) && !isFinite(R))
                return L; // Ambos ±Infinity — retorna Infinity/NegInfinito
        ...
        derivada: (fn, ponto, ordem = 1) => {
            ...
            if (ordem > 4)  throw new Error('calculo.derivada(): máximo suportado: ordem 4 (precisão numérica).');
        ...
        integral: (fn, a, b) => {
            ...
            for (let i = 1; i < n; i++) {
                const y = fn(a + i * h);
                if (typeof y !== 'number') throw new Error('calculo.integral(): fn deve retornar um número.');
                soma += (i % 2 === 0 ? 2 : 4) * y;
            }
            return (h / 3) * soma;
        },
    ...
    estatistica: {
        ...
        fatorial: n => {
            ...
            if (n > 170) throw new Error('estatistica.fatorial(): n muito grande (máx: 170).');
            return _fatorial(n);
        },
        ...
        combinacao: (n, k) => {
            ...
            if (n < 0 || k < 0)  throw new Error('estatistica.combinacao(): n e k devem ser ≥ 0.');
            if (k > n)            throw new Error('estatistica.combinacao(): k não pode ser maior que n.');
            if (n > 170)          throw new Error('estatistica.combinacao(): n muito grande (máx: 170).');
        ...
        arranjo: (n, k) => {
            ...
            if (n < 0 || k < 0)  throw new Error('estatistica.arranjo(): n e k devem ser ≥ 0.');
            if (k > n)            throw new Error('estatistica.arranjo(): k não pode ser maior que n.');
            if (n > 170)          throw new Error('estatistica.arranjo(): n muito grande (máx: 170).');
        ...
        moda: lista => {
            ...
            const modas = [];
            freq.forEach((f, v) => { if (f === maxFreq) modas.push(v); });
        ...
        variancia: lista => {
            ...
            return lista._v.reduce((s, v) => s + Math.pow(v - m, 2), 0) / lista._v.length;
        },
...
/* ============================================================
   9. PRÉ-PROCESSADOR — PARÂMETROS TIPADOS
      funcao f(inteiro(x >= 0), real(y) : 3.14) →
      function f(__p_x = undefined, __p_y = 3.14) {
          let x = __p_x; if (x === undefined) throw ...;
          if (!(x >= 0)) throw ...;
          let y = __p_y;
   ============================================================ */
...
function processarParametrosTipados(codigo) {
    return codigo.replace(
        /\bfuncao\s+([A-Za-z_]\w*)\s*\(([^)]*)\)\s*\{/g,
        (match, nome, paramStr) => {
            if (!/\b(inteiro|real|caracter|booleano)\s*\(/.test(paramStr)) return match;
            const params = _splitParams(paramStr);
            const jsParams = [], validations = [];

            params.forEach(raw => {
                raw = raw.trim();
                const m = raw.match(
                    /^(inteiro|real|caracter|booleano)\s*\(\s*([A-Za-z_]\w*)\s*([><=!]{1,2}=?\s*[^)]+?)?\s*\)\s*(?::\s*(.+))?$/
                );
                if (m) {
                    const [, , pn, restricao, defVal] = m;
                    const jsDef = defVal !== undefined ? _defVal(defVal.trim()) : '__OBRIG__';
                    const ip    = `__p_${pn}`;
                    jsParams.push(jsDef === '__OBRIG__' ? `${ip}=undefined` : `${ip}=${jsDef}`);
                    let vb = `let ${pn}=${ip};`;
                    if (jsDef === '__OBRIG__')
                        vb += `\nif(${pn}===undefined)throw new Error("${nome}: parâmetro '${pn}' é obrigatório.");`;
                    if (restricao) {
                        const r = restricao.trim().replace(/\bverdadeiro\b/g,'true').replace(/\bfalso\b/g,'false');
                        vb += `\nif(!(${pn} ${r}))throw new Error("${nome}: '${pn}' deve satisfazer: ${pn} ${restricao.trim().replace(/"/g,"'")}.");`;
                    }
                    validations.push(vb);
                } else {
                    jsParams.push(raw);
            ...
            return `function ${nome}(${jsParams.join(', ')}) {\n${validations.join('\n')}`;
        }
    );
}
...
function traduzirCodigo(fonte) {
    ...
    const wm = c.match(/^[ \t]*[A-Za-zÀ-ÖØ-öø-ÿ_]\w*[ \t]*\(\s*\)\s*\{([\s\S]*)\}[ \t]*$/);
    if (wm) c = wm[1];
    ...
    const S = '\x00';
    const tks = [];
    const p = s => { tks.push(s); return S + (tks.length - 1) + S; };
    ...
    c = c.replace(new RegExp(`(${tp})\\s*\\*\\s*([A-Za-z0-9_]+)`, 'g'), '$1.repeat($2)');
    c = c.replace(new RegExp(`([A-Za-z0-9_]+)\\s*\\*\\s*(${tp})`,  'g'), '$2.repeat($1)');
...
function limparConsole() {
    ...
    if (s) { s.textContent = ''; s.className = 'exec-status'; }
}
...
async function executarCodigo() {
    ...
    try {
        ...
        if (status) { status.textContent = 'erro'; status.className = 'exec-status error'; }
        return;
    }

    /* Contexto assíncrono — _EVAL_HEADER_LINES controla o offset de linha */
    const ctx =
        `(async function __A__(){\n` +           // linha 1
        `try{\n` +                               // linha 2  ← código do usuário começa aqui
        `${traduzido}\n` +
    ...
    try {
        await eval(ctx);
        if (status) { status.textContent = 'OK'; status.className = 'exec-status ok'; }
    } catch (erroEval) {
