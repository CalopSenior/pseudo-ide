...
const KW_CTRL   = new Set(['se','senao','enquanto','faca','para','tentar','capturar','funcao','retorno','quebrar','continuar','escolha','caso','padrao']);
const KW_VAL    = new Set(['verdadeiro','falso','vazio','indefinido','Infinito','NegInfinito']);
...
const KW_FN     = new Set([
    ...
    // estatistica
    'fatorial','combinacao','arranjo','media','mediana','moda','variancia','desvioPadrao',
    // mapa
    'definir',
]);
...
function aplicarHighlight(texto){
    ...
    while(i<len){
        ...
        if(!inBlockComment && ch==='/' && texto[i+1]==='*'){
            ...
            while(i<len){
                cmt+=texto[i];
                if(texto[i]==='*'&&texto[i+1]==='/'){cmt+=texto[i+1];i+=2;break;}
                i++;
            }
            // Renderiza mantendo quebras de linha
            const lines=cmt.split('\n');
            html+=lines.map((l,idx)=>`<span class="t-cmt">${esc(l)}</span>${idx<lines.length-1?'\n':''}`).join('');
            continue;
        }

        /* Comentário de linha // */
        if(ch==='/'&&texto[i+1]==='/'){
            let cmt=''; while(i<len&&texto[i]!=='\n') cmt+=texto[i++];
            html+=`<span class="t-cmt">${esc(cmt)}</span>`; continue;
        }

        /* f-strings */
        if(ch==='f'&&i+1<len&&(texto[i+1]==='"'||texto[i+1]==="'")){
            const q=texto[i+1]; let s='f'+q; i+=2;
            while(i<len&&texto[i]!==q){if(texto[i]==='\\'){s+=texto[i]+(texto[i+1]||'');i+=2;}else s+=texto[i++];}
            s+=(texto[i]||''); i++;
            html+=`<span class="t-str">${esc(s)}</span>`; continue;
        }

        /* Strings */
        if(ch==='"'||ch==="'"){
            let s=ch; i++;
            while(i<len&&texto[i]!==ch){if(texto[i]==='\\'){s+=texto[i]+(texto[i+1]||'');i+=2;}else s+=texto[i++];}
            s+=(texto[i]||''); i++;
            html+=`<span class="t-str">${esc(s)}</span>`; continue;
        }
        ...
        if(ch==='`'){
            let s='`'; i++;
            while(i<len&&texto[i]!=='`'){if(texto[i]==='\\'){s+=texto[i]+(texto[i+1]||'');i+=2;}else s+=texto[i++];}
            s+=(texto[i]||''); i++;
            html+=`<span class="t-str">${esc(s)}</span>`; continue;
        }
        ...
        if(/[0-9]/.test(ch)||(ch==='.'&&/[0-9]/.test(texto[i+1]||''))){
            ...
            while(i<len&&/[0-9.eE+\-]/.test(texto[i])){
                ...
                if((texto[i]==='-'||texto[i]==='+')&&!['e','E'].includes(n[n.length-1])) break;
                n+=texto[i++];
            }
            html+=`<span class="t-num">${esc(n)}</span>`; continue;
        }
...
function toggleFold(lineIdx){
    ...
    codeEditor.selectionStart=codeEditor.selectionEnd=Math.min(pos,codeEditor.value.length);
    atualizarEditor();
}
...
function atualizarEditor(){
    ...
    const lnHtml=linhas.map((linha,idx)=>{
        const temAbertura=/\{/.test(linha) && !/\}/.test(linha.replace(/.*\{/,''));
        ...
        const arrow=hasContent
            ...
            : '';
        return `<div class="ln-row">${arrow}<span class="ln-num">${idx+1}</span></div>`;
    }).join('');
    lineNumbers.innerHTML=lnHtml;

    // Eventos nos arrows
    lineNumbers.querySelectorAll('.fold-arrow').forEach(el=>{
        el.addEventListener('click',e=>{e.stopPropagation();toggleFold(+el.dataset.line);});
    });
...
const COMPLETIONS = [
    ...
    {label:'super',      detail:'tipo universal (aceita qualquer valor ou lista fixa [])'},
    // Valores
    ...
    {label:'importar mat como m',      detail:'snippet',insert:'importar mat como m;'},
    {label:'importar metodos como m',  detail:'snippet',insert:'importar metodos como m;'},
    {label:'importar tabular como t',  detail:'snippet',insert:'importar tabular como t;'},
    {label:'importar calculo como c',  detail:'snippet',insert:'importar calculo como c;'},
    {label:'importar estatistica como e',detail:'snippet',insert:'importar estatistica como e;'},
    ...
    {label:'m.aleatorio()',detail:'mat — int aleatório',      insert:'m.aleatorio('},
    {label:'m.somatorio()',detail:'mat — Σ fn(i)',             insert:'m.somatorio('},
    {label:'m.produtorio()',detail:'mat — Π fn(i)',           insert:'m.produtorio('},
    // calculo
    {label:'c.limite()',   detail:'calculo — lim x→p f(x)',  insert:'c.limite('},
    {label:'c.derivada()', detail:'calculo — f\'(x) ordem n',insert:'c.derivada('},
    {label:'c.integral()', detail:'calculo — ∫f dx de a a b',insert:'c.integral('},
    // estatistica
    {label:'e.fatorial()',    detail:'estat — n!',            insert:'e.fatorial('},
    {label:'e.combinacao()',  detail:'estat — C(n,k)',        insert:'e.combinacao('},
    ...
    {label:'e.variancia()',   detail:'estat — variância σ²',  insert:'e.variancia('},
    {label:'e.desvioPadrao()',detail:'estat — desvio σ',      insert:'e.desvioPadrao('},
    // tabular
    {label:'t.tabela()',        detail:'tabular — tabela HTML',         insert:'t.tabela('},
    {label:'t.separador()',     detail:'tabular — linha divisória',     insert:'t.separador()'},
    {label:'t.progresso()',     detail:'tabular — barra de progresso',  insert:'t.progresso('},
    {label:'t.tabelaVerdade()', detail:'tabular — tabela-verdade',      insert:'t.tabelaVerdade('},
    // metodos
...
let acList=[], acIdx=-1, acTriggerStart=0;
...
function showAutocomplete(){
    ...
    acList=COMPLETIONS.filter(c=>c.label.toLowerCase().startsWith(word.toLowerCase()));
...
function renderAutocomplete(pos,wordLen){
    ...
    acList.forEach((item,i)=>{
        ...
        li.addEventListener('mousedown',e=>{ e.preventDefault(); applyCompletion(i); });
    ...
    const top=editorRect.top+(currentLine*lineHeight)-scrollTop+lineHeight;
...
function hideAutocomplete(){ acPopup.style.display='none'; acList=[]; acIdx=-1; }
...
codeEditor.addEventListener('keydown', function(e){
    ...
    if(acList.length>0){
        ...
        if(e.key==='Tab'||e.key==='Enter'){
            if(acIdx>=0){ e.preventDefault(); applyCompletion(acIdx); return; }
        }
        if(e.key==='Escape'){ hideAutocomplete(); return; }
    }

    /* ---- Ctrl + S → Executar ---- */
    if(e.ctrlKey&&(e.key==='s'||e.key==='S')){ e.preventDefault(); executarCodigo(); return; }
    /* ---- Ctrl + M → Limpar ---- */
    if(e.ctrlKey&&(e.key==='m'||e.key==='M')){ e.preventDefault(); limparConsole(); return; }
    /* ---- F1 → Docs ---- */
    ...
    if(todos[e.key]){
        if(paresStr[e.key]&&val[ss]===e.key&&ss===se){
            e.preventDefault(); ta.selectionStart=ta.selectionEnd=ss+1; return;
        }
        e.preventDefault();
    ...
    if(e.key==='Enter'){
        ...
        if(extra&&proxCh==='}'){
            ins='\n'+indent+extra+'\n'+indent;
            ta.value=val.slice(0,ss)+ins+val.slice(se);
            ta.selectionStart=ta.selectionEnd=ss+indent.length+extra.length+1;
        } else {
            ins='\n'+indent+extra;
            ta.value=val.slice(0,ss)+ins+val.slice(se);
            ta.selectionStart=ta.selectionEnd=ss+ins.length;
...
function _getLineRange(val,ss,se){
    ...
    const fimLinha=se+(after.indexOf('\n')===-1?after.length:after.indexOf('\n'));
...
function _indentLines(ta,val,ss,se,dir){
    ...
    const novas=linhas.map(l=>{
        if(dir===1) return '    '+l;
        return l.startsWith('    ')?l.slice(4):l.startsWith('\t')?l.slice(1):l.replace(/^ {1,3}/,'');
    });
    const novo=novas.join('\n');
    const delta=novo.length-segmento.length;
    ta.value=val.slice(0,start)+novo+val.slice(end);
...
function _moveLine(ta,val,ss,se,dir){
    ...
    const inicioAlvo=linhas.slice(0,alvo).join('\n').length+(alvo>0?1:0);
...
function atualizarMetadados(texto){
    ...
    if(!hasAny){ bar.classList.add('hidden'); return; }
    bar.classList.remove('hidden');
    const setEl=(id,val)=>{
        const el=document.getElementById(id);
        if(el){ el.style.display=val?'':'none'; if(val) el.querySelector('.meta-val').textContent=val; }
    };
...
function salvarArquivo(){
    ...
    let nome=(m.nome||codeEditor.value.split('\n')[0].replace(/\(.*$/,'').trim()||'algoritmo')
        .replace(/[^a-zA-Z0-9_\-\s]/g,'').trim().replace(/\s+/g,'_')||'algoritmo';
    const blob=new Blob([codigo],{type:'text/plain;charset=utf-8'});
    const url=URL.createObjectURL(blob);
    const a=document.createElement('a'); a.href=url; a.download=nome+'.ide'; a.click();
    URL.revokeObjectURL(url);
}

function abrirArquivo(){ document.getElementById('file-import').click(); }

function _onFileImport(e){
    const file=e.target.files[0]; if(!file) return;
    const r=new FileReader();
    r.onload=ev=>{ codeEditor.value=ev.target.result; foldState.clear(); atualizarEditor(); codeEditor.focus(); };
...
(function(){
    ...
    document.addEventListener('mousemove',e=>{
        if(!dragging) return;
        const total=mainLayout.getBoundingClientRect().width-resizer.offsetWidth;
        const nw=Math.max(260,Math.min(total-220,startW+(e.clientX-startX)));
...
window.addEventListener('keydown',e=>{
    ...
    if(e.key==='Escape'){
        if(!document.getElementById('help-modal').classList.contains('hidden')) toggleHelp();
...
window.addEventListener('load',()=>{
    ...
    document.getElementById('file-import').addEventListener('change',_onFileImport);
});
