...
const Bibliotecas = {
  ...
  graficos: {
    ...
    interativo: (function () {
      ...
      function _isFunc(v) { return typeof v === "function"; }

      function _extrairPontosI(dado) {
        if (dado instanceof PseudoLista) return dado._v.map((p, i) => {
          if (p instanceof PseudoMapa) {
            const m = Object.fromEntries(p._v);
            return { x: +m.x, y: +m.y };
          }
          if (Array.isArray(p) && p.length >= 2) return { x: +p[0], y: +p[1] };
          if (p instanceof PseudoLista && p._v.length >= 2) return { x: +p._v[0], y: +p._v[1] };
          return null;
        }).filter(Boolean);
        if (Array.isArray(dado)) return dado.map(p => {
      ...
      function _normalizarPlotavel(item, idx) {
        if (item && typeof item === "object" && "_tipo" in item) {
          item._cor = item._opc.cor || CORES[idx % CORES.length];
          return item;
        }
        if (_isFunc(item)) {
          return { _tipo: "funcao", _dado: item, _cor: CORES[idx % CORES.length], _opc: {} };
        }
        const pts = _extrairPontosI(item);
        if (pts.length) {
          return { _tipo: "pontos", _dado: pts, _cor: CORES[idx % CORES.length], _opc: {} };
        }
        return null;
      }

      function _dominioInicial(plotaveis) {
        ...
        plotaveis.forEach(p => {
          ...
          if (p._tipo === "funcao") {
            const xRange = (hasData && isFinite(minX)) ? [minX, maxX] : [-10, 10];
            const step = (xRange[1] - xRange[0]) / 60;
            for (let xi = xRange[0]; xi <= xRange[1]; xi += step) {
              try {
                const y = p._dado(xi);
                if (isFinite(y)) { minY = Math.min(minY, y); maxY = Math.max(maxY, y); hasData = true; }
              } catch(e) {}
            }
            if (!isFinite(minX)) { minX = xRange[0]; maxX = xRange[1]; }
          }
        });
        if (!hasData || !isFinite(minX)) return { cx: 0, cy: 0, scale: 40 };
        const padX = (maxX - minX) * 0.12 || 1;
        const padY = (maxY - minY) * 0.12 || 1;
        const wx = maxX - minX + 2 * padX;
        const wy = maxY - minY + 2 * padY;
        const scale = Math.min(480 / wx, 320 / wy);
        return { cx: (minX + maxX) / 2, cy: (minY + maxY) / 2, scale: Math.max(5, Math.min(scale, 200)) };
      }

      function _criarWidget(plotaveis, cfg) {
        const W = cfg.largura || 560, H = cfg.altura || 360;
        const dpr = window.devicePixelRatio || 1;

        const wrap = document.createElement("div");
        wrap.style.cssText = "position:relative;display:inline-block;user-select:none;font-family:monospace;";

        const canvas = document.createElement("canvas");
        canvas.width  = W * dpr;
        canvas.height = H * dpr;
        canvas.style.cssText = `width:${W}px;height:${H}px;display:block;cursor:grab;border-radius:6px;`;
        wrap.appendChild(canvas);

        const tooltip = document.createElement("div");
        tooltip.style.cssText = "position:absolute;background:#1a1a2e;color:#e2e8f0;padding:3px 8px;border-radius:4px;font-size:11px;pointer-events:none;display:none;border:1px solid #444;white-space:nowrap;z-index:10;";
        wrap.appendChild(tooltip);

        const ctx = canvas.getContext("2d");
        ctx.scale(dpr, dpr);

        const state = { cx: 0, cy: 0, scale: 40 };
        const dominio = _dominioInicial(plotaveis);
        state.cx = dominio.cx; state.cy = dominio.cy; state.scale = dominio.scale;

        const toScreen = (wx, wy) => ({
          sx: W / 2 + (wx - state.cx) * state.scale,
          sy: H / 2 - (wy - state.cy) * state.scale
        });
        const toWorld = (sx, sy) => ({
          wx: state.cx + (sx - W / 2) / state.scale,
          wy: state.cy - (sy - H / 2) / state.scale
        });
        ...
        function draw() {
          ...
          // grid
          const worldL = toWorld(0, 0).wx, worldR = toWorld(W, 0).wx;
          const worldT = toWorld(0, 0).wy, worldB = toWorld(0, H).wy;
          const xRange = worldR - worldL, yRange = worldT - worldB;
          const stepX = _niceStep(xRange), stepY = _niceStep(yRange);

          ctx.strokeStyle = "#1e2030"; ctx.lineWidth = 1;
          for (let xi = Math.ceil(worldL / stepX) * stepX; xi <= worldR + stepX; xi += stepX) {
            const { sx } = toScreen(xi, 0);
            ctx.beginPath(); ctx.moveTo(sx, 0); ctx.lineTo(sx, H); ctx.stroke();
          }
          for (let yi = Math.floor(worldB / stepY) * stepY; yi <= worldT + stepY; yi += stepY) {
            const { sy } = toScreen(0, yi);
          ...
          // axis labels
          ctx.fillStyle = "#666a8a"; ctx.font = "10px monospace"; ctx.textAlign = "center";
          for (let xi = Math.ceil(worldL / stepX) * stepX; xi <= worldR; xi += stepX) {
            if (Math.abs(xi) < stepX * 0.01) continue;
            const { sx, sy } = toScreen(xi, 0);
            const labelY = (sy > 10 && sy < H - 16) ? sy + 12 : (sy <= 10 ? 12 : H - 6);
            ctx.fillText(_fmtNum(xi), sx, labelY);
          }
          ctx.textAlign = "right";
          for (let yi = Math.floor(worldB / stepY) * stepY; yi <= worldT; yi += stepY) {
            if (Math.abs(yi) < stepY * 0.01) continue;
            const { sx, sy } = toScreen(0, yi);
            const labelX = (ox.sx > 28 && ox.sx < W) ? ox.sx - 4 : (ox.sx <= 28 ? ox.sx + 24 : W - 4);
            ctx.fillText(_fmtNum(yi), labelX, sy + 3);
          }

          // plotaveis
          plotaveis.forEach(p => {
            if (!p) return;
            ctx.strokeStyle = p._cor; ctx.fillStyle = p._cor;
            ...
            if (p._tipo === "funcao") {
              ...
              for (let sx = 0; sx <= W; sx += 1) {
                ...
                let y;
                try { y = p._dado(wx); } catch(e) { pen = false; continue; }
                if (!isFinite(y)) { pen = false; continue; }
                const sy = toScreen(wx, y).sy;
            ...
            if (p._tipo === "serie") {
              const wxL = toWorld(0, 0).wx, wxR = toWorld(W, 0).wx;
              const xiMin = Math.ceil(wxL), xiMax = Math.floor(wxR);
              const r = p._opc.raio || 4;
              let prevSx = null, prevSy = null;
              if (p._opc.linha !== false) ctx.lineWidth = 1.5;
              for (let xi = xiMin; xi <= xiMax; xi++) {
                let y;
                try { y = p._dado(xi); } catch(e) { prevSx = null; continue; }
                if (!isFinite(y)) { prevSx = null; continue; }
                const { sx, sy } = toScreen(xi, y);
                if (p._opc.linha !== false && prevSx !== null) {
          ...
          // legend
          const legendItems = plotaveis.filter(p => p && p._opc.rotulo);
          if (legendItems.length) {
            let lx = W - 10, ly = 10;
            legendItems.forEach(p => {
              const lbl = p._opc.rotulo;
              const tw = ctx.measureText(lbl).width;
              ctx.fillStyle = "rgba(15,17,23,0.8)";
              ctx.fillRect(lx - tw - 24, ly, tw + 28, 18);
              ctx.fillStyle = p._cor;
              ctx.fillRect(lx - tw - 20, ly + 5, 10, 8);
              ctx.fillStyle = "#cdd6f4"; ctx.font = "11px monospace"; ctx.textAlign = "left";
              ctx.fillText(lbl, lx - tw - 6, ly + 13);
              ly += 22;
            });
          }
        }
        ...
        window.addEventListener("mousemove", e => {
          if (drag) {
            ...
            state.cx = drag.cx - dx / state.scale;
            state.cy = drag.cy + dy / state.scale;
            schedDraw();
          }
        });
        ...
        canvas.addEventListener("wheel", e => {
          ...
          state.scale *= factor;
          state.cx = wx - (mx - W / 2) / state.scale;
          state.cy = wy + (my - H / 2) / state.scale;
          schedDraw();
        }, { passive: false });
        ...
        canvas.addEventListener("touchstart", e => {
          if (e.touches.length === 1) {
            lastTouch = { x: e.touches[0].clientX, y: e.touches[0].clientY, cx: state.cx, cy: state.cy };
          }
        }, { passive: true });
        canvas.addEventListener("touchmove", e => {
          if (e.touches.length === 1 && lastTouch) {
            ...
            state.cx = lastTouch.cx - dx / state.scale;
            state.cy = lastTouch.cy + dy / state.scale;
            schedDraw();
          }
        }, { passive: false });
        ...
        canvas.addEventListener("mousemove", e => {
          ...
          const mx = e.clientX - rect.left, my = e.clientY - rect.top;
          const { wx, wy } = toWorld(mx, my);
          let nearest = null, nearD = 12;

          plotaveis.forEach(p => {
            if (!p) return;
            if (p._tipo === "funcao") {
              let y; try { y = p._dado(wx); } catch(e) { return; }
              if (!isFinite(y)) return;
              const { sy } = toScreen(wx, y);
              const d = Math.abs(sy - my);
              if (d < nearD) { nearD = d; nearest = { x: wx, y, cor: p._cor }; }
            }
            if (p._tipo === "pontos") {
              p._dado.forEach(pt => {
                const { sx, sy } = toScreen(pt.x, pt.y);
                const d = Math.hypot(sx - mx, sy - my);
                if (d < nearD) { nearD = d; nearest = { x: pt.x, y: pt.y, cor: p._cor }; }
              });
            }
            if (p._tipo === "serie") {
              const xi = Math.round(wx);
              let y; try { y = p._dado(xi); } catch(e) { return; }
              if (!isFinite(y)) return;
              const { sx, sy } = toScreen(xi, y);
              const d = Math.hypot(sx - mx, sy - my);
              if (d < nearD) { nearD = d; nearest = { x: xi, y, cor: p._cor }; }
            }
          });

          if (nearest) {
            tooltip.style.display = "block";
            tooltip.style.borderColor = nearest.cor;
            tooltip.textContent = `(${_fmtNum(nearest.x)}, ${_fmtNum(nearest.y)})`;
            const tx = Math.min(mx + 12, W - tooltip.offsetWidth - 4);
      ...
      function plotar(dado, opcoes) {
        const o = _opc(opcoes);
        if (_isFunc(dado)) return { _tipo: "funcao", _dado: dado, _cor: o.cor || null, _opc: o };
        const pts = _extrairPontosI(dado);
        return { _tipo: "pontos", _dado: pts, _cor: o.cor || null, _opc: o };
      }

      function serie(funcao, opcoes) {
        const o = _opc(opcoes);
        return { _tipo: "serie", _dado: funcao, _cor: o.cor || null, _opc: o };
      }

      function grafico(plotaveis, cfg) {
        cfg = _opc(cfg);
        let arr;
        if (plotaveis instanceof PseudoLista) arr = plotaveis._v;
        else if (Array.isArray(plotaveis)) arr = plotaveis;
        else arr = [plotaveis];

        const normalized = arr.map((item, idx) => _normalizarPlotavel(item, idx)).filter(Boolean);
        const widget = _criarWidget(normalized, cfg);
